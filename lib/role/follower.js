        ...
        if (rpc.message instanceof AppendEntriesRPC) {
            if (rpc.message.term == this.client.state.currentTerm) {
                ...
                if (rpc.message.prevLogIndex <= this.client.state.log.length - 1) {
                    ...
                    this.client.rpc.message(rpc.message.leaderId, new AppendEntriesRPCReply(this.client.state.currentTerm, true, this.client.state.log.length - 1))
                } else {
                    // this.client.rpc.message(rpc.message.leaderId, new AppendEntriesRPCReply(this.client.state.currentTerm, false, rpc.message.prevLogIndex - 1))
                }
